...
Luôn dùng `@sth87/shadcn-design-system` thay vì tự xây dựng UI primitives. Xem chi tiết API trong **shadcn-design-system skill**.

```typescript
// Tree-shakeable (ưu tiên cho production)
import { Button } from "@sth87/shadcn-design-system/button";
// Import đầy đủ (cũng hợp lệ)
...
- Dùng utility **`cn()`** (từ design system) cho class điều kiện / merge class
...
- **CSS variables** cho theming (định nghĩa trong `theme.css` của design system)
