...
function SlideshowEditorPreview({
  items,
  p,
  style,
}: {
...
function ThumbnailsEditorPreview({
  items,
  p,
}: {
  items: GalleryItem[];
...
function Carousel3DEditorPreview({
  items,
  p,
}: {
  items: GalleryItem[];
...
function SwiperSliderRuntime({
  ...
  // For cards: use padding-bottom trick so the wrapper's layout height = slideHeight + cardsPad.
  // This means the component's bounding box encompasses the FULL visual stack, not just the active card.
  //   - Parse aspect ratio string (e.g. "16/9") → height percentage relative to width
  //   - Wrapper: height:0 + paddingBottom:calc(heightPct% + cardsPad) → layout height = slideHeight + cardsPad
  //   - Swiper: position:absolute filling only the top (slideHeight) portion (100% - cardsPad)
  //   - Stacked cards overflow visible into the reserved cardsPad zone below
  ...
  const wrapperStyle: React.CSSProperties = isCards
    ? {
        // padding-bottom trick: total layout height = slideHeight + cardsPad
        // paddingBottom percentage is relative to width, so heightPct% = height/width ratio
        position: "relative",
        width: "100%",
        height: 0,
  ...
  // Swiper style: for cards, absolutely positioned so height = total - cardsPad (slide area only).
  // Stacked cards paint outside the Swiper box via overflow:visible, into the cardsPad zone.
          ...
          <SwiperSlide key={img.id ?? i} style={{ position: "relative", overflow: "hidden" }}>
            <div
              ...
              style={{
                ...
                height: "100%",
                transform: cc.parallax ? "translateX(-10%)" : undefined, // 👈 center lại
              }}
            >
              {img.link ? (
                <a
...
export const GalleryProComponent: ComponentDefinition = {
  ...
  runtimeRenderer: ({ node, style }) => {
    ...
    const isCarousel = ["slider", "slideshow", "carousel-3d"].includes(layoutMode);
    return (
      // Cards mode: inner SwiperSliderRuntime uses padding-bottom trick so full stack height
      // is in layout flow — no overflow:visible needed on this outer wrapper.
      <div style={{ ...s }}>{renderByMode(layoutMode, items, p, cc, false, s)}</div>
    );
  },
};
