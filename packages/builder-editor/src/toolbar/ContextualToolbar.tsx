...
import { TOOLTIP_DELAY_MS, GALLERY_LAYOUT_MODES } from "@ui-builder/shared";
...
import { GallerySettingsPanel } from "../panels/gallery";
...
export const ContextualToolbar: React.FC<ContextualToolbarProps> = ({ nodeId, rect, zoom, panOffset, onDelete, onDuplicate, onMoveUp, onMoveDown, onDragHandlePointerDown, onOpenMediaManager, onOpenGalleryManager }) => {
  ...
  if (isGalleryNode) {
    const layoutMode = String(node.props["layoutMode"] ?? "grid");
    const modeLabel = GALLERY_LAYOUT_MODES.find((m) => m.value === layoutMode)?.label ?? layoutMode;
    breadcrumb = `Gallery · ${modeLabel}`;
  }
      ...
      >
        ...
        {isGalleryNode && (
          <>
            ...
            {/* Settings button — unified GallerySettingsPanel for all modes */}
            <Tooltip>
              <TooltipTrigger asChild>
                <Button
                  ref={gallerySettingsBtnRef}
                  variant="ghost"
                  size="icon-sm"
                  className="h-6 w-6"
                  onClick={(e) => {
              ...
              <TooltipContent side="top">Gallery Settings</TooltipContent>
            </Tooltip>

            {/* Unified Gallery Settings panel — Layout tab shows all modes, Design tab adapts */}
            {gallerySettingsOpen && node && createPortal(
              <FloatingPanel
