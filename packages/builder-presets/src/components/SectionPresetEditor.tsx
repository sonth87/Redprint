...
const TABS: Array<{ id: PanelTab; label: string; Icon: React.ElementType }> = [
  ...
  { id: "layout", label: "Layout & Slots", Icon: SlidersHorizontal },
];

export interface SectionPresetEditorProps {
  item: PaletteItem;
 ...
 */
export function SectionPresetEditor({
  item,
  registry,
  onReset,
  onChange,
  allPresets = [],
}: SectionPresetEditorProps) {
...
function SectionPresetEditorInner({
  item,
  registry,
  onReset,
  onChange,
                ...
                {activeTab === "layout" && (
                  <div className="flex-1 overflow-y-auto p-4 space-y-6">
                    <div className="space-y-3">
                      <h3 className="font-medium text-sm">Layout Variant</h3>
                      <p className="text-[11px] text-muted-foreground">
                        Switch the structural layout of this section. (WIP: Maps roles to new wrapper structure)
                      </p>
                      <div className="grid grid-cols-2 gap-2">
                        {["split-left", "split-right", "centered", "media-background"].map((l) => {
                           const isActive = item.layoutVariant === l || (!item.layoutVariant && l === "centered");
                           return (
                             <div 
                               key={l} 
                               className={`border rounded p-2 text-[11px] font-medium text-center cursor-pointer transition-colors ${
                                 isActive ? "bg-primary text-primary-foreground border-primary" : "hover:bg-muted text-muted-foreground"
                               }`}
                               onClick={() => {
                                 onChange?.({ ...documentToPaletteItem(item, state.document), layoutVariant: l });
                               }}
                             >
                               {l}
                             </div>
                    ...
                    <div className="space-y-3">
                      ...
                      <p className="text-[11px] text-muted-foreground">
                        Define which slots this section provides to the user.
                      </p>
                      <div className="space-y-2">
                        {["content", "media", "actions", "background"].map((slot) => (
                          <div key={slot} className="flex justify-between items-center text-xs px-2.5 py-2 border rounded bg-muted/30">
                             <span className="font-medium text-foreground">{slot}</span>
                             <span className="text-[10px] text-muted-foreground italic">Template</span>
                          </div>
                        ))}
                      </div>
                    </div>
