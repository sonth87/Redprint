...
> **Codex CLI**: Read the following files in order for full project context:

1. **`.Codex/ARCHITECTURE.md`** — Project-specific architecture & type contracts (always read first)
2. **`.Codex/RULES.md`** — Baseline rules & conventions (shared across all projects)
...
> **Language**: English (`.Codex/`). Vietnamese translations available in `.Codex.vi/`.
